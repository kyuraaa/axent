...
function ShaderPlane() {
  const material = useMemo(() => new CPPNShaderMaterial(), []);

  useFrame((state) => {
    if (!material) return;
    (material as any).iTime = state.clock.elapsedTime;
    const { width, height } = state.size;
    (material as any).iResolution.set(width, height);
  });
...
export function NeuralNetworkShaderBackground() {
  ...
  if (!webGLSupported) {
    return (
      <div 
        ...
        style={{ 
          background: 'linear-gradient(135deg, hsl(var(--primary)/0.3) 0%, hsl(var(--background)) 50%, hsl(var(--accent)/0.2) 100%)',
